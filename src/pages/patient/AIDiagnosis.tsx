...
export default function AIDiagnosis() {
  ...
  const API_KEY = import.meta.env.VITE_RAPIDAPI_KEY || '0a86719367mshbc9953321881f46p100df4jsne23856f3a5f3';
  ...
  const localHeuristic = (symptoms: string[]) => {
    return AI_DISEASES_DB.map((disease) => {
      const matchingSymptoms = disease.symptoms.filter((s) =>
        symptoms.some((selected) => selected.toLowerCase() === s.toLowerCase())
      );
      const confidence =
        matchingSymptoms.length / Math.max(disease.symptoms.length, symptoms.length);
  ...
  // Medical diagnosis API integration
  const getAIPredictions = async (symptoms: string[]) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}?noqueue=1`, {
        method: 'POST',
        headers: {
      ...
      // The exact structure might need adjustment based on the actual API response
      // Check if the response contains a 'diagnoses' array or similar
      const results = data.diagnoses || data.results || [];
      
      if (Array.isArray(results) && results.length > 0) {
        return results
          .map((item: any) => ({
            disease: item.diagnosis || item.condition || item.name || "Unknown Condition",
            confidence: item.confidence 
              ? Math.min(1, Math.max(0, parseFloat(item.confidence) / 100)) 
              : 0.5,
            ...(item.icd10 && { icd10: item.icd10 }),
            ...(item.description && { description: item.description })
          }))
          .sort((a: any, b: any) => b.confidence - a.confidence)
          .slice(0, 5);
      } else if (data.diagnosis) {
        // Handle case where the response has a single diagnosis
        return [{
          disease: data.diagnosis,
          confidence: 0.8, // Default confidence if not provided
          ...(data.icd10 && { icd10: data.icd10 })
        }];
      }
      ...
      // Return null to trigger fallback to local heuristic
      return null;
    }
  };
  ...
  const analyzeSymptoms = async () => {
    ...
    setAnalyzing(true);
    let predictions = await getAIPredictions(selectedSymptoms);
    if (!predictions) {
      predictions = localHeuristic(selectedSymptoms);
    }
